...
export const corvidConspiracy: Faction = {
  ...
  mechanics: 'The Corvids revolve around Plot tokens. You have four types: Bombs (destroy all enemy pieces), Snares (trap enemy movement), Extortion (steal cards), and Raids (spawn warriors when removed). You place these plots face-down on the map using the Plot action. To score, you must Flip a plot in a clearing where you have a warrior, scoring 1 VP for every face-up plot you have on the map (including the one just flipped). Enemies can challenge your face-down plots with the Exposure action. If they guess the plot type correctly by showing a matching card, they remove the plot and score 1 VP. If they are wrong, you steal their card. This creates a high-stakes game of bluffing. Your Embedded Agents ability also makes you a tough defender, dealing an extra hit in battle if you have a face-down plot in the clearing.',
  howToPlay: 'The Corvid Conspiracy is a faction of spies and saboteurs focused on bluffing and misdirection. Your core gameplay loop involves placing face-down Plot tokens on the map, then flipping them with a warrior present to score points and trigger an effect. The scoring is exponential; the more plots already face-up, the more points the next one scores. This creates a tense game of cat and mouse, where you try to outwit your opponents and protect your valuable plots until you can reveal them for a massive point gain.',
  strategy: 'Your entire game is about mind games. Place plots in locations that are most threatening to your opponents. A Bomb plot in their most valuable clearing forces them to either risk an Exposure or move out. A Snare can lock down a key path. Since you score for *each* face-up plot when you flip a new one, your scoring can explode in the late game. Use your Nimble ability to move warriors into position to flip plots. Your "Embedded Agents" ability makes enemies think twice about attacking you, as they\'ll take an extra hit if you have a face-down plot in the clearing. Try to trick opponents into exposing the wrong plot to steal their cards.',
  specialAbilities: [
